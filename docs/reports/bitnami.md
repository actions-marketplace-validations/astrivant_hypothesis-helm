...
- [Appendix: graph structure](#appendix-graph-structure)
...
Started (Unix epoch): 1790199081
Started (UTC): 2026-09-23T21:31:21.000+00:00
Finished (UTC): 2026-09-24T00:35:21.716+00:00
Run fingerprint (SHA-256): `0e8d71cca2d59c213c1af4111496708467ace8c3ea3b08995ba5a08ff1d0af96`
Versions: Hypothesis 6.168.0; hypothesis-helm 0.1.0
Elapsed (wall clock): 9766.64 seconds
Chart testing: 9354.20 seconds
Dependency preparation: 395.42 seconds (excluded from testing budgets)
Charts discovered: 115
Manifest test attempts: 2741
...
38 time-limit; 8 failed; 1 skipped-library; 1 interrupted; 67 pending.

## Settings

Filtering: True | Seed: 0 | Traversal: random
Chart timeout: 180.0 seconds | Workers: 8
...
```bash
hypothesis-helm test third_party/bitnami-charts --filter --disable-codes HH2006 --jobs 8 --chart-timeout 180s --scan-timeout 200m --timeout 50 --max-examples 5 --seed 0 --shard none --max-mutations 96 --pca-samples 8 --pca-timeout 60s --artifact-dir .cache/hypothesis-helm/bitnami-runs --report docs/reports/bitnami.md --log-color
```
...
29 distinct diagnostics across 43 occurrences; 14 repeats grouped.
...
Status: time-limit | Attempts: 22

No failing test cases were recorded. The time limit was reached before testing finished.

Coverage: sampled values paths; full interaction coverage was not established.
...
Status: failed | Attempts: 62

Coverage: generated path tests. Finite interaction coverage was unavailable: Cannot enumerate the input domain: permutations need closed
objects at (); set additionalProperties: false. The sampled paths do not establish N-way coverage.

Audit findings: 452. Full paths and template references: [JSON](<bitnami-data/0006.audit.json.gz>).
...
- 446 additional audit findings in [JSON](<bitnami-data/0006.audit.json.gz>).
...
#### E026 ([HH1109](#hh1109---invalid-manifest-field-type))
...
#### E027 ([HH3002](#hh3002---incompatible-value-type-in-template))
...
```text
[HH3002] Error: argo-workflows/charts/mysql/templates/networkpolicy.yaml:72:69 executing
"argo-workflows/charts/mysql/templates/networkpolicy.yaml" at <$value.port>: can't evaluate field port in type interface {}
```
...
Changed overrides (used together):
- `$.mysql.enabled = true (was false)`
- ``$.mysql.primary.service.extraPorts = [{"aaaa": [], "Oaoaaa": {"aa\raa": {"g4aa": false, "Qaaa": {"Wiaa:aaaaaaaYaaaaaaaa": false}, "a`aV":
  null}, "": [], " a"... [value shortened]``
...
Audit findings: 224. Full paths and template references: [JSON](<bitnami-data/0007.audit.json.gz>).
...
Audit findings: 192. Full paths and template references: [JSON](<bitnami-data/0008.audit.json.gz>).
...
Audit findings: 309. Full paths and template references: [JSON](<bitnami-data/0009.audit.json.gz>).
...
Status: time-limit | Attempts: 6

No failing test cases were recorded. The time limit was reached before testing finished.

Coverage: sampled values paths; full interaction coverage was not established.

Audit findings: 404. Full paths and template references: [JSON](<bitnami-data/0010.audit.json.gz>).
...
Status: time-limit | Attempts: 15

No failing test cases were recorded. The time limit was reached before testing finished.

Coverage: sampled values paths; full interaction coverage was not established.

Audit findings: 643. Full paths and template references: [JSON](<bitnami-data/0011.audit.json.gz>).
...
Audit findings: 1163. Full paths and template references: [JSON](<bitnami-data/0012.audit.json.gz>).
...
Audit findings: 507. Full paths and template references: [JSON](<bitnami-data/0013.audit.json.gz>).
...
Audit findings: 276. Full paths and template references: [JSON](<bitnami-data/0014.audit.json.gz>).
...
Audit findings: 423. Full paths and template references: [JSON](<bitnami-data/0015.audit.json.gz>).
...
Audit findings: 470. Full paths and template references: [JSON](<bitnami-data/0017.audit.json.gz>).
...
Audit findings: 255. Full paths and template references: [JSON](<bitnami-data/0018.audit.json.gz>).
...
Audit findings: 591. Full paths and template references: [JSON](<bitnami-data/0019.audit.json.gz>).
...
Audit findings: 166. Full paths and template references: [JSON](<bitnami-data/0020.audit.json.gz>).
...
Status: time-limit | Attempts: 57

No failing test cases were recorded. The time limit was reached before testing finished.

Coverage: sampled values paths; full interaction coverage was not established.

Audit findings: 346. Full paths and template references: [JSON](<bitnami-data/0021.audit.json.gz>).
...
Audit findings: 1181. Full paths and template references: [JSON](<bitnami-data/0022.audit.json.gz>).
...
Audit findings: 319. Full paths and template references: [JSON](<bitnami-data/0023.audit.json.gz>).
...
Audit findings: 224. Full paths and template references: [JSON](<bitnami-data/0024.audit.json.gz>).
...
Audit findings: 814. Full paths and template references: [JSON](<bitnami-data/0025.audit.json.gz>).
...
Status: failed | Attempts: 23

Coverage: generated path tests. Finite interaction coverage was unavailable: Cannot enumerate the input domain: permutations need closed
objects at (); set additionalProperties: false. The sampled paths do not establish N-way coverage.

Audit findings: 325. Full paths and template references: [JSON](<bitnami-data/0026.audit.json.gz>).
...
- 319 additional audit findings in [JSON](<bitnami-data/0026.audit.json.gz>).

#### E024 ([HH1105](#hh1105---missing-resource-name))
...
Status: failed | Phase: `$.certgen.serviceAccount.name`
...
Audit findings: 407. Full paths and template references: [JSON](<bitnami-data/0027.audit.json.gz>).
...
Status: failed | Attempts: 1658

Coverage: generated path tests. Finite interaction coverage was unavailable: Cannot enumerate the input domain: permutations need closed
objects at (); set additionalProperties: false. The sampled paths do not establish N-way coverage.

Audit findings: 402. Full paths and template references: [JSON](<bitnami-data/0028.audit.json.gz>).
...
- 396 additional audit findings in [JSON](<bitnami-data/0028.audit.json.gz>).

Configuration rejections: 0 excluded; 38 adjusted and tested; 38 Helm verification renders (separate from manifest-test attempts).
...
Status: failed | Phase: `$.extraEnvVarsSecret`
...
Status: failed | Phase: `$.aws.credentials.secretName`
...
Status: failed | Phase: `$.image.pullSecrets`
...
Status: failed | Phase: `$.global.imagePullSecrets`
...
Status: failed | Phase: `$.image.repository`

Changed overrides (used together):
- `$.global.security.allowInsecureImages = true (was false)`
- `$.image.repository = "\r" (was "bitnami/external-dns")`

Renderer random inputs (replay tape in artifacts):

Status: failed | Phase: `$.global.imageRegistry`
...
Status: failed | Phase: `$.excludeDomains`
...
Status: failed | Phase: `$.zoneIdFilters[*]`
...
Status: failed | Phase: `$.regexDomainExclusion`
...
5 additional occurrences are retained in the JSON report and chart artifacts.
...
Status: failed | Phase: `$.aws.zoneTags`
...
Status: failed | Phase: `$.aws.dynamodbTable`
...
#### E028 ([HH3003](#hh3003---undefined-named-template))
...
```text
[HH3003] external-dns/templates/_helpers.tpl:716:16 executing "external-dns.serviceAccountName" at <include "common.names.fullname" .>:
error calling include: template: no template "common.names.fullname" associated with template "gotpl"
```

Status: failed | Phase: `$.tags`
...
Audit findings: 281. Full paths and template references: [JSON](<bitnami-data/0029.audit.json.gz>).
...
Audit findings: 240. Full paths and template references: [JSON](<bitnami-data/0030.audit.json.gz>).
...
Status: failed | Attempts: 48

Coverage: generated path tests. Finite interaction coverage was unavailable: Cannot enumerate the input domain: permutations need closed
objects at (); set additionalProperties: false. The sampled paths do not establish N-way coverage.

Audit findings: 427. Full paths and template references: [JSON](<bitnami-data/0031.audit.json.gz>).
...
- 421 additional audit findings in [JSON](<bitnami-data/0031.audit.json.gz>).
...
Status: failed | Phase: `$.forwarder.livenessProbe.tcpSocket.port`
...
Audit findings: 1042. Full paths and template references: [JSON](<bitnami-data/0032.audit.json.gz>).
...
Status: failed | Attempts: 70

Coverage: generated path tests. Finite interaction coverage was unavailable: Cannot enumerate the input domain: permutations need closed
objects at (); set additionalProperties: false. The sampled paths do not establish N-way coverage.

Audit findings: 262. Full paths and template references: [JSON](<bitnami-data/0033.audit.json.gz>).
...
- 256 additional audit findings in [JSON](<bitnami-data/0033.audit.json.gz>).
...
Status: failed | Phase: `$.mysql.primary.extraPodSpec.initContainers[*].livenessProbe.httpGet.httpHeaders[*]`

Changed overrides (used together):
- `$.mysql.primary.extraPodSpec.initContainers.__hypothesis_key__.livenessProbe.httpGet.httpHeaders.__hypothesis_key__ = null`

Absent from overrides:

```text
$.mysql.primary.extraPodSpec.initContainers["*"].livenessProbe.httpGet.httpHeaders["*"]
```
...
Audit findings: 241. Full paths and template references: [JSON](<bitnami-data/0034.audit.json.gz>).
...
Audit findings: 266. Full paths and template references: [JSON](<bitnami-data/0035.audit.json.gz>).
...
Audit findings: 306. Full paths and template references: [JSON](<bitnami-data/0036.audit.json.gz>).
...
Audit findings: 292. Full paths and template references: [JSON](<bitnami-data/0037.audit.json.gz>).
...
Audit findings: 195. Full paths and template references: [JSON](<bitnami-data/0038.audit.json.gz>).
...
Audit findings: 1386. Full paths and template references: [JSON](<bitnami-data/0039.audit.json.gz>).
...
Audit findings: 1540. Full paths and template references: [JSON](<bitnami-data/0040.audit.json.gz>).
...
Audit findings: 277. Full paths and template references: [JSON](<bitnami-data/0041.audit.json.gz>).
...
- 271 additional audit findings in [JSON](<bitnami-data/0041.audit.json.gz>).
...
Audit findings: 999. Full paths and template references: [JSON](<bitnami-data/0042.audit.json.gz>).
...
Audit findings: 186. Full paths and template references: [JSON](<bitnami-data/0043.audit.json.gz>).
...
Audit findings: 1499. Full paths and template references: [JSON](<bitnami-data/0044.audit.json.gz>).
...
Status: failed | Attempts: 318

Coverage: generated path tests. Finite interaction coverage was unavailable: Cannot enumerate the input domain: permutations need closed
objects at (); set additionalProperties: false. The sampled paths do not establish N-way coverage.

Audit findings: 345. Full paths and template references: [JSON](<bitnami-data/0045.audit.json.gz>).
...
- 339 additional audit findings in [JSON](<bitnami-data/0045.audit.json.gz>).
...
Audit findings: 404. Full paths and template references: [JSON](<bitnami-data/0046.audit.json.gz>).
...
Audit findings: 328. Full paths and template references: [JSON](<bitnami-data/0047.audit.json.gz>).
...
Status: interrupted | Phase: chart
...
Each point represents one chart with published graph measurements. The upper panel compares vertices with connections. The lower counts
