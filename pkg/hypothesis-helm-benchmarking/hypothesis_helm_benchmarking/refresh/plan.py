...
def source_path(project: Path) -> str:
    """
    Expose Python imports from the dashed project folders in a source checkout.
    ...
    return os.pathsep.join(
        str(project / "pkg" / folder) for folder in ("hypothesis-helm", "hypothesis-helm-catalog", "hypothesis-helm-benchmarking", "")
    )
...
class Refresh:
    ...
    def operations(self) -> tuple[Operation, ...]:
        ...
        def stage(name: str, *requires: str, exclusive: bool = False, allow_failure: bool = False) -> None:
            ...
            """
            recipe = (
                Path("pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/operations.sh")
