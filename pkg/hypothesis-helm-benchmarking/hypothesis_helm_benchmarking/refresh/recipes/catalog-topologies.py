    ...
    json.dumps(
        {
            ...
            "charts": rows,
            "contract": "Exact invariants of the exported graph; dynamic access remains explicitly unresolved.",
        },
        indent=2,
    )
    + "\n"
...
lines = [
    ...
    "deterministically. Geometry shows the dependency layout. Dependency-chain length counts edges along a directed path.",
    "",
    "C is the number of weak components. The cycle rank E − V + C is the first Betti number of the one-dimensional "
    "underlying undirected multigraph. Parallel reference edges count separately. These invariants describe graph "
    "structure; runtime effects require separate measurements. Opaque template access remains unresolved.",
    "",
    "A rendered baseline is one observation. Static-only graphs could not obtain that baseline; missing-values and "
    "incomplete jobs are explicit. Unresolved access remains marked as unknown.",
    "",
