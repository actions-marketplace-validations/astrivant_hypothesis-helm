...
from hypothesis_helm_benchmarking.studies.calibration import verify_sweep
from hypothesis_helm_benchmarking.studies.error_surface import METHODS, METRICS, grid_values, verify
...
for study in STUDIES:
    ...
    if study == "sensitivity-ordering":
        ...
        verify_ordering(result)
        for name in FIGURES:
            for extension in ("png", "svg"):
                assert (directory / f"{name}.{extension}").is_file()
        for structure in result["fixtures"]:
            for suffix in ("chart.json", "observations.json.gz"):
                assert (directory / f"{structure}-{suffix}").is_file()
            assert (directory / "cases" / f"{structure}-parameters.yaml").is_file()
    if study == "matrix":
