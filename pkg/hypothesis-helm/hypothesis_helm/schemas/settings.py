...
def validate_settings(raw: dict[str, object]) -> dict[str, object]:
    ...
    if "hypothesis" in result:
        ...
        if set(parameters) - {"max_examples", "deadline_ms", "phases", "suppress_health_check"}:
            raise ValueError(
                "hypothesis accepts character_sets, control_characters, exclude_characters, "
                "max_examples, deadline_ms, phases and suppress_health_check"
            )
...
def generation_settings(chart: Path) -> dict[str, object]:
    ...
    """
    from hypothesis_helm.schemas.selectors import matching_rules, source_identity
