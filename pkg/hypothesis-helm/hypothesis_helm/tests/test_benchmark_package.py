...
def test_benchmark_wheel(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [poetry, "build", "--format", "wheel", "--output", str(tmp_path / "addon")],
        cwd=root / "pkg/hypothesis-helm-benchmarking",
        env=environment,
        check=True,
        capture_output=True,
        text=True,
    ...
    with ZipFile(next((tmp_path / "addon").glob("*.whl"))) as archive:
        ...
        requirements = metadata.get_all("Requires-Dist", [])
        assert all(
            any(line.startswith(dependency + " ") for line in requirements) for dependency in ("hypothesis-helm", "matplotlib", "numpy")
        )
        entry_points = archive.read(metadata_name.replace("METADATA", "entry_points.txt")).decode()
