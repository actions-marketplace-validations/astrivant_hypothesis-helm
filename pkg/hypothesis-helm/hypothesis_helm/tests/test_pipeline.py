...
import pytest
from pipeline import Control, Estimate, Outcome, Scheduler, ShortestRemaining, Statistics, Work, checkpoints
...
def test_feedback_graph_repeats_and_pauses_between_rounds(tmp_path: Path) -> None:
    ...
    """
    from pipeline import FeedbackGraph, Graph
...
def test_shutdown_checkpoints_then_finalizes(tmp_path: Path) -> None:
    ...
    """
    from pipeline import Finalizer, ShutdownContract
...
def test_feedback_shutdown_reaches_nested_worker(tmp_path: Path) -> None:
    ...
    """
    from pipeline import FeedbackGraph, Finalizer, Graph, ShutdownContract
...
def test_graph_traversal_dispatch(tmp_path: Path, depth_first: bool) -> None:
    ...
    """
    from pipeline import BreadthFirst, DepthFirst
    ...
    def record(name: str) -> Outcome:
        """
        Record dispatch order and complete the fixture workload.

        Args:
            name (str): Dispatched workload identity.

        Returns:
            Outcome: Successful fixture completion.
        """
        observed.append(name)
        return Outcome()

    units = [Unit(work, lambda *_, name=work.name: record(name)) for work in works]
    Scheduler(
        units,
        slots=1,
        directory=tmp_path,
