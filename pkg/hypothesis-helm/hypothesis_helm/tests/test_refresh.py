...
def test_refresh_repository_recipe(tmp_path: Path) -> None:
    ...
    initialized = subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/initialize.py"),
            str(run_root),
        ],
        cwd=tmp_path,
        env=environment,
...
def test_refresh_requires_complete_stress_matrix(tmp_path: Path, damage: str | None) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/verify-measurements.py"),
            str(tmp_path),
        ],
        capture_output=True,
        text=True,
...
def test_refresh_includes_every_stress_topology(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/prepare-fixtures.py"),
            str(tmp_path),
        ],
        check=True,
        capture_output=True,
    ...
    subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/prepare-topologies.py"),
            str(tmp_path),
        ],
        check=True,
        capture_output=True,
...
def test_topology_retries_include_recipe_files(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/plan-topology-retries.py"),
            str(tmp_path),
        ],
        check=True,
        capture_output=True,
...
def test_publish_groups_studies(tmp_path: Path) -> None:
    ...
    """
    project = Path(__file__).resolve().parents[4]
    recipes = project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes"
    ...
    published = tmp_path / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/assets"
...
def test_refresh_scans_follow_published_diagrams(tmp_path: Path, failure: str | None) -> None:
    ...
    (root / "operations.sh").write_text(
        (project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/operations.sh").read_text()
    )
...
def test_refresh_summaries_replace_numbers_and_preserve_prose(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    project = Path(__file__).resolve().parents[4]
    script = project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/update-documentation.py"
...
def test_refresh_dispatches_all_studies(tmp_path: Path, symbolic: str) -> None:
    ...
    root = tmp_path / "refresh"
    root.mkdir()
    shutil.copy2(project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/studies.sh", root / "studies.sh")
    binary = tmp_path / "bin"
    binary.mkdir()
    ...
    for study in STUDIES:
        subprocess.run(
            [
                "bash",
                str(project / "pkg/hypothesis-helm-benchmarking/hypothesis_helm_benchmarking/refresh/recipes/operations.sh"),
                study,
                str(root),
            ],
            env=environment,
