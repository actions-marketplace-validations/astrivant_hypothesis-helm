...
def strategy_source(schema: dict[str, object], *, generation: dict[str, object] | None = None, path: tuple[str | int, ...] = ()) -> str:
    ...
    Args:
        ...
        generation (dict[str, object] | None): Saved per-field generation settings.
        path (tuple[str | int, ...]): Path of this fragment in the chart values.
    ...
    if kind == "string" and set(node) <= {"type", "minLength", "maxLength"}:
        return (
            f"st.text(alphabet=st.characters(codec={'ascii' if selected == 'ascii' else 'utf-8'!r}, "
            "exclude_categories=('Cc', 'Cs'), include_characters='\\n\\r'), "
    ...
    *,
    ...
    inventory = {
        ...
        "paths": [
            dict(
                asdict(p),
                strategy=strategy_source(p.schema, generation=generation, path=p.path),
                hypothesis=hypothesis_parameters(generation, p.path, max_examples),
            )
    ...
    for entry in model.paths:
        ...
        parameters = hypothesis_parameters(generation, entry.path, max_examples)
        health = ", ".join(f"HealthCheck.{name}" for name in sequence(parameters["suppress_health_check"]))
        phases = ", ".join(f"Phase.{name}" for name in sequence(parameters["phases"]))
        strategy = strategy_source(entry.schema, generation=generation, path=entry.path)
        text_filter = "" if custom_text(generation) else ".filter(supported_generated_text)"
        lines += [
            f"# Path: {entry.path!r}; contract: {entry.origin}",
            f"@pytest.mark.hypothesis_helm_path({entry.path!r})",
            f"@settings(max_examples={parameters['max_examples']}, deadline={parameters['deadline_ms']!r}, "
            f"phases=({phases},), suppress_health_check=[{health}])",
            f"@given(value=({strategy}){text_filter}, data=st.data())",
