...
class CandidateChecks:
    ...
    def _check(self, values: dict[str, object], *, force_render: bool = False, baseline: bool = False) -> bool:
        """
        Evaluate a candidate within its scoped finding policy.
        ...
        try:
            with execution_timer(self.remaining_time()):
                ...
                if not baseline and not validators.validator_for(self.chart.input_domains().apply({}))(
                    self.chart.input_domains().apply({})
                ...
                rejection = self.policy.predict(effective) if self.policy is not None and not baseline else None
                if rejection is not None and self.policy is not None and not self.policy.preserves(rejection):
                    ...
                    if rejection is not None:
                        ...
                        replacement = self.policy.repair(
                            ...
                            lambda candidate: (
                                validators.validator_for(self.chart.generation_schema())(self.chart.generation_schema()).is_valid(
                            ...
                            allow_enum_changes=self.finite_values is None,
                        )
                ...
                if not resources and not self.allow_empty:
                    check_rule(False, "HH1107", "chart rendered no resources (use allow_empty explicitly)")
            ...
            if self.policy is not None and exc.code != "HH2007":
                ...
                if (
                    ...
                    and matches_rejection(str(exc), declared_rejection)
                ):
