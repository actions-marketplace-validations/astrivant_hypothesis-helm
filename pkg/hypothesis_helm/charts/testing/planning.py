...
def build_plan(
    ...
    *,
    ...
    finite_values = enumerate_values(chart.generation_schema(), options.max_cases) if options.exhaustive else None
    ...
    if finite_values is not None:
        ...
        distinct: list[int] = []
        domain_validator = validators.validator_for(chart.generation_schema())(chart.generation_schema())
        for index, values in enumerate(finite_values):
            if not domain_validator.is_valid(json_value(merge_values(chart.defaults, values))):
                continue
    ...
    if options.sampling.percent < 100:
        LOGGER.info(
            "Random sampling: %s/%s retained; %s omitted; minimum %s; recall depends on selected inputs",
