...
def check_chart(
    ...
    *,
    ...
    sensitivity_order: int | None = None,
    ...
    Args:
        ...
        sensitivity_order (int | None): Maximum sensitivity group order, bounded by permutations.
        ...
        input_inventory (InputInventory | None): Shared compiler inventory, already announced by the owning chart coordinator.
    ...
    if input_inventory is None:
        input_baseline(chart.path, len(inputs.known))
    ...
    if plan.dry_report is not None:
        if sensitivity_order is not None:
            plan.dry_report["sensitivity"] = {"order": sensitivity_order, "status": "deferred until execution", "extra_test_cases": 0}
    ...
    findings = FindingLog(chart_name(chart.path), chart.defaults, protected_paths, artifact_dir)
    ...
    def save_failure(exc: BaseException) -> dict[str, object]:
        ...
        if result["status"] == "failed":
            findings.emit(
                "Counterexample recorded" if isinstance(exc, (RenderFailure, AssertionError)) else "Testing diagnostic",
    ...
    if sensitivity_order is not None:
        ...
        try:
            sensitivity = SensitivityOrder(finite_values, chart.defaults, sensitivity_order, remaining_time)
            finite_values = sensitivity.initial()
            checks.observe = sensitivity.observe
            LOGGER.info("Sensitivity traversal: order %d, %d retained reference cases", sensitivity_order, len(sensitivity.prefix))
    ...
    if expansion is not None:
        ...
        for position, index in enumerate(work):
            ...
            if failed:
                ...
                if sensitivity is not None:
                    try:
                        additional = sensitivity.rank(additional)
                    except TimeLimitReached:
            ...
            expansion_checked += 1
            expansion_executed += int(position >= initial_count)
            if sensitivity is not None and position == len(sensitivity.prefix):
                try:
                    ranked = sensitivity.rank(select(expansion_values, work[position + 1 : initial_count]))
                    work[position + 1 : initial_count] = [expansion_positions[configuration_key(value)] for value in ranked]
                except TimeLimitReached:
                    return stopped_report()
    ...
    if finite_values is not None:
        try:
            if expansion is None:
                if jobs == 1:
                    for values in sensitivity.traverse() if sensitivity is not None else finite_values:
                        check(values)
                else:
    ...
    max_examples = int(str(parameters["max_examples"]))
    configured_health = tuple(HealthCheck[str(name)] for name in sequence(parameters["suppress_health_check"]))

    @seed(random_seed)
    @settings(
        max_examples=max_examples,
        deadline=float(str(parameters["deadline_ms"])) if parameters["deadline_ms"] is not None else None,
        database=None,
        phases=(Phase.generate,) if fail_fast else tuple(Phase[str(name)] for name in sequence(parameters["phases"])),
