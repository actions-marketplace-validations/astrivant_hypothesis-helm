...
def order_paths(  # noqa: UP047 - pinned pydocstyle 6 cannot parse PEP 695 function type parameters.
    ...
    *,
    ...
    if strategy == "sensitivity-first":
        raise ValueError("sensitivity-first requires finite --permutations testing, not individual path properties")
    if strategy == "linear":
        return list(items)
...
def order_configurations(
    ...
    *,
    ...
    if strategy == "linear":
        return values
    if strategy in {"random", "sensitivity-first"}:
        return select(
            values,
            order_paths(
                range(len(values)), lambda index: (), strategy="random", seed=seed, identity=lambda index: configuration_key(values[index])
            ),
        )
