...
from hypothesis_helm.schemas.generation.combinations import InteractionPlan
...
class PermutationStatistics:
    ...
    def finish(self, status: str, error: str | None = None) -> dict[str, object]:
        ...
        stats = self.snapshot()
        stats["exit_code"] = (
            0 if status in {"passed", "ignored", "findings"} else 130 if status == "interrupted" else 124 if status == "time-limit" else 1
        )
        ...
        if destination is not None:
            ...
            suite = ET.Element(
                "testsuite",
                {
                    ...
                    "skipped": "1" if status in {"ignored", "findings"} else "0",
            ...
            if status != "passed":
                failure = ET.SubElement(
                    case,
                    "skipped" if status in {"ignored", "findings"} else "error" if status in ("interrupted", "time-limit") else "failure",
