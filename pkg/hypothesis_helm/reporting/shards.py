...
def aggregate(inputs: list[Path], total: int, run_id: str, output: Path | None = None) -> int:
    ...
    Args:
        ...
        output (Path | None): New immutable output directory; defaults to docs/reports/aggregate.
    ...
    output = (output or Path("docs/reports/aggregate")).resolve()
    ...
    with ExitStack() as scope:
        ...
        reused = 0
        failures: list[str] = []
        failure_phases: list[dict[str, object]] = []
        ...
        # Shards overlap in time: use their wall-clock envelope, not a sum of durations.
        record["elapsed_seconds"] = report["elapsed_seconds"]
        with TemporaryDirectory(prefix=f".{output.name}-", dir=output.parent) as temporary:
            ...
            report["artifact_checksums"] = {
                ...
                for name in ("junit.xml", "report.md", "report.pdf", "report-overview.png")
            }
