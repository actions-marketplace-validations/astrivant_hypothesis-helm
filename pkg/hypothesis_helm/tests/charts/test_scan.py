...
def test_scan_execution(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert argument_parser().parse_args(["test", "--log-file", "/dev/stderr", str(tmp_path), "--report"]).report == ""
...
def test_interrupt_preserves_remaining_charts(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str], returned: bool
) -> None:
    """
    ...
    Args:
        ...
        returned (bool): Whether the chart catches cancellation and returns a partial result.
    ...
    def interrupt(path: Path, args: object, artifacts: Path) -> dict[str, object]:
        ...
        Returns:
            dict[str, object]: Partial result if the chart handles the interrupt internally.
        """
        if returned:
...
def test_scan_deadline_preserves_runner_statistics(
    ...
    def exercise(path: Path, args: Namespace, artifacts: Path) -> dict[str, object]:
        ...
        Args:
            path (Path): Chart under test.
            args (Namespace): Execution configuration.
            artifacts (Path): Statistics destination.

        Returns:
            dict[str, object]: Counters preserved after an execution alarm.
        """
        # Expire only after this fake runner has started; machine load must not consume the setup budget.
        args.scan_deadline = time.monotonic() - 1
        return {
            "status": "time-limit",
            "attempts": 3,
