...
def test_json_manifest_stream(tmp_path: Path, capfd: pytest.CaptureFixture[str], mode: str) -> None:
    ...
    if mode == "paths":
        ...
        assert main(["run", str(tmp_path), "--output-format", "json", "--match", "replicas"]) == 0
...
def test_parallel_workers_and_large_manifest_stream(tmp_path: Path, capfd: pytest.CaptureFixture[str], output_format: str) -> None:
    """
    ...
    Args:
        ...
        output_format (str): Selected resource serialization, shared by both workers.

    Returns:
        None: Both workers overlap and every large resource is preserved.
    """
...
"""
    ...
    assert main(["run", str(tmp_path), "-j", "2", "-o", output_format]) == 0
    output = capfd.readouterr()
    documents = [json.loads(line) for line in output.out.splitlines()] if output_format == "json" else load_all(output.out)
    assert all(isinstance(resource, dict) for resource in documents)
    resources = [resource for resource in documents if isinstance(resource, dict)]
