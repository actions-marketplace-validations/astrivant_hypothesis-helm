...
def test_fail_requires_source_fields(tmp_path: Path, command: str, capsys: pytest.CaptureFixture[str]) -> None:
    """
    ...
    assert main(["generate", "--log-file", "/dev/stderr", str(chart), "--output", str(suite)]) == 0
    capsys.readouterr()
    original = (chart / "values.yaml").read_bytes()
    arguments = [command, str(suite if command == "run" else chart), "--fail", "--log-file", "/dev/stderr"]
    if command in ("test", "run"):
        arguments += ["--dry-run"]
    if command == "generate":
        arguments += ["--output", str(tmp_path / "fail-suite")]
    ...
    assert not (tmp_path / "fail-suite").exists()
