...
from hypothesis_helm_benchmarking.refresh.plan import source_path
...
def test_refresh_repository_recipe(tmp_path: Path) -> None:
    ...
    environment = dict(
        os.environ,
        PYTHONPATH=source_path(project),
        PATH=f"{Path(sys.executable).parent}{os.pathsep}{os.environ['PATH']}",
        HYPOTHESIS_HELM_JOB_INDEX="1",
        HYPOTHESIS_HELM_JOB_TOTAL="3",
    )
    run_root = Path(".cache/benchmarks/refresh-1234")
    initialized = subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis_helm_benchmarking/refresh/recipes/initialize.py"),
            str(run_root),
        ],
        cwd=tmp_path,
        env=environment,
    ...
    assert json.loads((tmp_path / run_root / "provenance.json").read_text())["selection_order"] == SELECTION_ORDER
    assert (tmp_path / ".cache/benchmarks/latest-refresh.txt").read_text().strip() == str(run_root)
    snapshot = tmp_path / run_root / "frozen-source"
    # Both native implementations and their module locks must survive transfer
    # to another CI runner, along with the Lua pass and regenerated inventories.
    for name in (
        "pkg/hypothesis_helm/compiler/assets/renderer/main.go",
        "pkg/hypothesis_helm/compiler/assets/renderer/go.mod",
        "pkg/hypothesis_helm/compiler/assets/renderer/go.sum",
        "pkg/hypothesis_helm/compiler/assets/builtin_inventory.json",
        "pkg/hypothesis_helm/compiler/lua/bounds.lua",
        "pkg/hypothesis_helm_catalog/upstream/main.go",
        "pkg/hypothesis_helm_catalog/upstream/helm.go",
        "pkg/hypothesis_helm_catalog/upstream/kubernetes.go",
        "pkg/hypothesis_helm_catalog/upstream/go.mod",
        "pkg/hypothesis_helm_catalog/upstream/go.sum",
        "pkg/hypothesis_helm_catalog/data/input-domains.json",
    ):
        assert (snapshot / name).read_bytes() == (project / name).read_bytes()
    ...
    for name in ("bitnami", "prometheus"):
        ...
        assert "HH2006" in worker["ignored_rules"]
        assert worker["settings"]["jobs"] == 6
        assert worker["settings"]["filter"] is True
        assert worker["settings"]["chart_timeout_seconds"] == 300
        assert worker["settings"]["max_examples"] == 10
        assert worker["settings"]["seed"] == 0
        primary = next(record for record in worker["charts"] if record["chart"] == ".")
        primary.update(
            traversal={
                "visited_order": [["replicas"]],
                "remaining_order": [],
                "visited_paths": 1,
                "remaining_paths": 0,
                "selected_paths": 1,
            ...
            seed=0,
            filtering={"order": "discover, filter, traverse, execute"},
        )
        ...
        assert rejected.returncode != 0 and "expected selection order" in rejected.stderr
        primary["filtering"]["order"] = SELECTION_ORDER
        worker_path.write_text(json.dumps(worker))
        previous = tmp_path / "docs/reports" / name
        previous.parent.mkdir(parents=True, exist_ok=True)
        previous.with_suffix(".md").write_text(f"# Previous scan\n\n![Overview](<{name}-overview.png>)\n")
        previous.with_suffix(".pdf").write_bytes(b"previous PDF")
        previous.with_name(f"{name}-overview.png").write_bytes(b"previous measurements")
        ...
        assert previous.with_name(f"{name}-overview.png").read_bytes().startswith(b"\x89PNG")
        assert (tmp_path / run / "previous-report" / f"{name}-previous-overview.png").read_bytes() == b"previous measurements"
        assert f"<{name}-previous-overview.png>" in (tmp_path / run / "previous-report" / f"{name}-previous.md").read_text()
        verified = json.loads((tmp_path / run / "verification.json").read_text())
        assert verified["completed_jobs"] == verified["expected_charts"] == 1
        assert verified["charts_with_verified_unique_traversal"] == 1
...
def test_refresh_requires_complete_stress_matrix(tmp_path: Path, damage: str | None) -> None:
    ...
    for study in studies:
        ...
        if study == "sensitivity-ordering":
            from hypothesis_helm_benchmarking.execution.ordering import Observation, methods, replay
            from hypothesis_helm_benchmarking.reporting.ordering import FIGURES

            observations = [Observation({"a": False}, [{"a": False}], ()), Observation({"a": True}, [{"a": True}], ("bug",))]
            mapping(document["metadata"]).update(status="complete", structures=["interactions"], permutations=1, seeds=[0])
            document["fixtures"] = {"interactions": {"cases": 2, "bug_orders": {"bug": 1}, "paths": {"a": ["a"]}}}
            rows = [{"structure": "interactions", **replay(observations, {"bug": 1}, method, 0)} for method in methods(1)]
            (directory / "cases").mkdir()
            (directory / "cases/interactions-parameters.yaml").write_text("parameters: {}\n")
            for filename in ("interactions-chart.json", "interactions-observations.json.gz"):
                (directory / filename).write_bytes(b"x" * 1001)
            for figure in FIGURES:
                for extension in ("png", "svg"):
                    (directory / f"{figure}.{extension}").write_bytes(b"x" * 1001)
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis_helm_benchmarking/refresh/recipes/verify-measurements.py"),
            str(tmp_path),
        ],
        capture_output=True,
        text=True,
...
def test_refresh_includes_every_stress_topology(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis_helm_benchmarking/refresh/recipes/prepare-fixtures.py"),
            str(tmp_path),
        ],
        check=True,
        capture_output=True,
    ...
    subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis_helm_benchmarking/refresh/recipes/prepare-topologies.py"),
            str(tmp_path),
        ],
        check=True,
        capture_output=True,
...
def test_topology_retries_include_recipe_files(tmp_path: Path) -> None:
    """
    Retry failed synthetic recipes alongside chart exports and unfinished drawings.

    Args:
        tmp_path (Path): Inventory containing each supported source and output state.

    Returns:
        None: Missing-values charts and complete graphs stay skipped; recipes are retried.
    """
    project = PROJECT_ROOT
    records = []
    for name in ("recipe", "compressed", "chart", "plot", "complete", "missing"):
        source = tmp_path / "sources" / name
        source.parent.mkdir(exist_ok=True)
        if name in {"recipe", "compressed"}:
            source = source.with_suffix(".yaml")
            source.write_text("parameters: {}\n")
        else:
            source.mkdir()
            if name != "missing":
                (source / "values.yaml").write_text("{}\n")
        output = tmp_path / "outputs/chart-topologies/synthetic" / name
        output.mkdir(parents=True)
        if name == "compressed":
            (output / "graph.json.gz").touch()
        if name == "plot":
            (output / "graph.json").write_text("{}\n")
        if name == "complete":
            for filename in ("metrics.json", "topology.png", "topology.svg"):
                (output / filename).touch()
        records.append({"repository": "synthetic", "chart": name, "source": str(source), "output": str(output)})
    (tmp_path / "topology-inventory.json").write_text(json.dumps(records))
    subprocess.run(
        [
            sys.executable,
            str(project / "pkg/hypothesis_helm_benchmarking/refresh/recipes/plan-topology-retries.py"),
            str(tmp_path),
        ],
        check=True,
        capture_output=True,
    )
    retries = (tmp_path / "topology-retry-charts.tsv").read_text().splitlines()
    plots = (tmp_path / "topology-retry-plots.tsv").read_text().splitlines()
    assert retries == [f"{item['source']}\t{item['output']}" for item in records[:3]]
...
def test_publish_groups_studies(tmp_path: Path) -> None:
    ...
    for name in (*studies, "chart-topologies"):
        ...
        (output / "plot.png").write_bytes(b"plot")
        (output / "cases").mkdir()
    ...
    assert not (tmp_path / "docs/reports/studies").exists()
    for name in (*studies, "chart-topologies"):
        directory = tmp_path / "studies" / name
        assert {path.name for path in directory.iterdir()} == {"README.md", "plot.png"}
        assert "Raw measurements (local run data)" in (directory / "README.md").read_text()
        assert json.loads((run / "outputs" / name / "results.json").read_text()) == {"study": name}
...
def test_refresh_scans_follow_published_diagrams(tmp_path: Path, failure: str | None) -> None:
    ...
    (root / "operations.sh").write_text((project / "pkg/hypothesis_helm_benchmarking/refresh/recipes/operations.sh").read_text())
...
def test_refresh_summaries_replace_numbers_and_preserve_prose(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    project = PROJECT_ROOT
    script = project / "pkg/hypothesis_helm_benchmarking/refresh/recipes/update-documentation.py"
    ...
    scans = {name: f".cache/refresh/refresh-1234/repositories/{name}-charts_1234" for name in ("bitnami", "prometheus")}
    (root / "provenance.json").write_text(json.dumps({"repository_scans": scans}))
    presented: list[str] = []
    monkeypatch.setattr(
        "hypothesis_helm.reporting.reports.repository.write_reports", lambda report, stem, **kwargs: presented.append(str(stem))
    )
    ...
    assert all(f"{directory}/README.md" not in readme.read_text() for directory in scans.values())
    assert "docs/reports/bitnami.md" in readme.read_text()
...
def test_refresh_dispatches_all_studies(tmp_path: Path, symbolic: str) -> None:
    ...
    for study in STUDIES:
        subprocess.run(
            [
                "bash",
                str(project / "pkg/hypothesis_helm_benchmarking/refresh/recipes/operations.sh"),
                study,
                str(root),
            ],
            env=environment,
