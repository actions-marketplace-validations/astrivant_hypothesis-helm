...
def test_memory_snapshot(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    refresh_env()
    monkeypatch.setattr(
        "hypothesis_helm.schemas.kubernetes.conformity.Processes.run", lambda *args, **kwargs: subprocess.CompletedProcess([], 0, "ext4\n")
    )
    with pytest.raises(ValueError, match="Linux tmpfs"):
        conformity.memory_snapshot(snapshot)
    assert not root.exists()
    monkeypatch.setattr(
        "hypothesis_helm.schemas.kubernetes.conformity.Processes.run", lambda *args, **kwargs: subprocess.CompletedProcess([], 0, "tmpfs\n")
    )
