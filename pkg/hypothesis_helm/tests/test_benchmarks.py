...
from hypothesis_helm_benchmarking.charts.workload import (
    expected_output,
    partition_indices,
    standard_values,
)
from hypothesis_helm_benchmarking.execution.runner import Job, execute_worker
...
def test_wrong_output_fails_instead_of_becoming_a_representative(
    ...
    monkeypatch.setattr(
        "hypothesis_helm_benchmarking.execution.runner.render",
...
def test_budget_limited_render_timeout(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, remaining: float, timeout: bool) -> None:
    ...
    """
    generate(tmp_path, input_complexity=8)
    monkeypatch.setattr("hypothesis_helm_benchmarking.execution.runner.time.perf_counter", lambda: 100.0)
    monkeypatch.setattr("hypothesis_helm_benchmarking.execution.runner.execution_timer", lambda seconds: nullcontext())
    ...
    monkeypatch.setattr("hypothesis_helm_benchmarking.execution.runner.render", render)
...
def test_linear_prefix_checkpoints_commit_only_completed_inputs(
    ...
    monkeypatch.setattr("hypothesis_helm_benchmarking.execution.runner.render", render)
    monkeypatch.setattr("hypothesis_helm_benchmarking.execution.runner.expected_output", oracle)
