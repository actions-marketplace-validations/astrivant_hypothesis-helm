...
def test_expansion_scheduler_and_benchmark(expansion_chart: Chart, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("hypothesis_helm_benchmarking.studies.expansion.render", render)
...
def test_expansion_cli_dry_run(expansion_chart: Chart, capsys: pytest.CaptureFixture[str], options: list[str]) -> None:
    ...
    assert main(["test", "--log-file", "/dev/stderr", str(expansion_chart.path), "--expand-failures", "--whole-chart"]) == 2
...
def test_structure_depth_sweep(expansion_chart: Chart, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from hypothesis_helm_benchmarking.studies.structure_depth import sweep
    ...
    monkeypatch.setattr("hypothesis_helm_benchmarking.studies.expansion.render", observed)
...
def test_pca_presets_expand_only_observed_failures(expansion_chart: Chart) -> None:
    ...
    from hypothesis_helm_benchmarking.analysis.selection import PRESETS
    from hypothesis_helm_benchmarking.studies.pca import expand_selections
