...
from hypothesis_helm_benchmarking.charts.fixture import FixtureWorkspace, case_path, chart_path, read_spec
from hypothesis_helm_benchmarking.charts.generator import generate, reproduce
from hypothesis_helm_benchmarking.charts.stress import FAMILIES, SIGNALS, Stress, progression
...
@pytest.mark.parametrize(("name", "settings"), progression(Stress()))
def test_stress_recipe_replays_every_step(tmp_path: Path, name: str, settings: Stress) -> None:
    """
    Replay every retained stress recipe, including YAML integer subclasses for zero.

    Args:
        tmp_path (Path): Isolated original chart, recipe and replay locations.
        name (str): Deterministic stress progression step.
        settings (Stress): Controls recorded in that step's recipe.

    Returns:
        None: Replayed charts have the same source bytes as their original generation.
    """
    original = tmp_path / name
    generate(original, input_complexity=12, stress=settings)
    replayed = tmp_path / "replayed"
    reproduce(original / "benchmark-parameters.yaml", replayed)
    assert source_digest(replayed) == source_digest(original)


@pytest.mark.parametrize("value", [False, True, 0.0, "0", -1, 3])
def test_stress_rejects_invalid_integer_controls(value: object) -> None:
    """
    Reject noninteger or out-of-range controls after accepting YAML integer subclasses.

    Args:
        value (object): Invalid coupled-pair control.

    Returns:
        None: Invalid controls fail before generating chart files.
    """
    with pytest.raises(ValueError, match="coupled_pairs must be an integer"):
        Stress(coupled_pairs=cast(int, value))
...
def test_cli_keeps_shard_recipes_separate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from hypothesis_helm_benchmarking.studies import performance as benchmark_helm
