...
def test_matrix_strategy_contracts(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, structure: str) -> None:
    ...
    monkeypatch.setattr(
        "hypothesis_helm_benchmarking.studies.matrix.render",
    ...
    monkeypatch.setattr("hypothesis_helm_benchmarking.studies.matrix.render", lambda *args, **kwargs: [])
    failed = measure(
        chart,
        spec,
        "default",
...
def test_matrix_distinguishes_deadline_from_render_timeout(
    ...
    clock = [0.0]
    monkeypatch.setattr("hypothesis_helm_benchmarking.studies.matrix.time.perf_counter", lambda: clock[0])
    monkeypatch.setattr("hypothesis_helm_benchmarking.studies.matrix.execution_timer", lambda seconds: nullcontext())
    ...
    monkeypatch.setattr("hypothesis_helm_benchmarking.studies.matrix.render", timeout)
