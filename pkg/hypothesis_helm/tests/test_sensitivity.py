...
def test_dense_sensitivity_study(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    from hypothesis_helm.benchmarking.studies.sensitivity import main as study
    ...
    assert len(json.loads((output / "mutations.json").read_text())) == 10
    assert "values.yaml" in json.loads((output / "chart-inputs.json").read_text())
