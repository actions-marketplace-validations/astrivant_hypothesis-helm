...
from hypothesis_helm_benchmarking.charts.topology import expected_topology, validate_topology
...
from hypothesis_helm_benchmarking.execution.cancellation import Cancellation, initialize
...
def execute_worker(job: Job) -> dict[str, object]:
    ...
    try:
        ...
        with execution_timer(job.deadline - started):
            ...
            with BenchmarkProgress(f"Replica {os.getpid()}: checks") as display:
                for index in display.track(job.indices):
