...
def read_json(path: Path) -> object:
    ...
    Args:
        path (Path): Expected uncompressed artifact location.

    Returns:
        object: Decoded artifact.
    """
    if path.exists():
...
summary = [
    ...
    "Inferred inputs describe template access. The authored schema defines the chart contract. Findings require triage; "
    ...
    "",
    f"[Combined PDF]({name}.pdf). Raw measurements, logs and resume records remain in the local run cache.",
]
phase_summary = []
...
summary.extend(
    [
        "",
        f"Generated text uses {settings.get('input_policy', {}).get('character_sets', 'unicode')}. "
        "Tabs and C0/C1 controls are excluded except LF and CR; authored text and explicit finite domains are unchanged. "
...
def portable(value: object) -> object:
    ...
    Args:
        value (object): Nested report content.

    Returns:
        object: Content with portable artifact paths.
    """
...
def generated_delta_supported(value: object, baseline: object) -> bool:
    """
    Check changed text and new keys without rejecting untouched supplied defaults.

    Args:
        value (object): Generated configuration value.
        baseline (object): Corresponding supplied default.

    Returns:
        bool: Whether generated differences use supported text.
    """
...
for record in records:
    ...
    if record.get("traversal"):
        ...
        assert record["filtering"]["order"] == SELECTION_ORDER, (
            f"{record['chart']}: expected selection order {SELECTION_ORDER!r}, received {record['filtering']['order']!r}"
        )
...
if previous_path.exists():
    ...
    previous_data = Path(".cache/refresh")
    candidates = sorted(path for path in previous_data.glob(f"*/repositories/{name}-charts_*/scan.json*") if path.parent != run)
    if candidates:
    ...
    archive = run / "previous-report" / f"{name}-{previous_epoch}"
    archive.parent.mkdir(parents=True, exist_ok=True)
    if not archive.with_suffix(".md").exists():
        archived_content = previous_path.read_text().replace(f"]({name}.pdf)", f"]({archive.name}.pdf)")
        overview = previous_path.with_name(f"{name}-overview.png")
        if overview.exists():
            archived_overview = archive.with_name(f"{archive.name}-overview.png")
            shutil.copyfile(overview, archived_overview)
...
This run supplies the combined report and PDF at `docs/reports/{name}.md` and `docs/reports/{name}.pdf`.
