...
set -euo pipefail
if [[ "${1:-}" == --help ]]; then
    echo 'Usage: hypothesis-helm-benchmark smoke [NEW_OUTPUT_DIRECTORY]'
...
output="${1:-.cache/benchmarks/smoke}"
export MPLBACKEND=Agg
mkdir -p "$(dirname "$output")"
mkdir "$output"
MPLCONFIGDIR="$(
    pushd "$output" >/dev/null
    printf '%s/matplotlib' "$PWD"
    popd >/dev/null
)"
export MPLCONFIGDIR
hypothesis-helm-benchmark stress --generate-only --output "$output/plan"
hypothesis-helm-benchmark stress --steps 3 --time-limit 1s --output "$output/stress"
hypothesis-helm-benchmark error-surface --input-complexity 3 \
...
hypothesis-helm audit --log-file /dev/stderr "$output/chart" --export-topological-graph "$output/graph.json" >"$output/audit.json"
hypothesis-helm-benchmark topology --graph "$output/graph.json" \
    --output "$output/topology" --title 'Combined topology stress fixture'
hypothesis-helm test --log-file /dev/stderr "$output/chart" --permutations 2 \
