...
from hypothesis_helm_benchmarking.charts.fixture import chart_path as resolve_chart
...
def plot(output: Path, rows: list[dict[str, object]], defects: list[Fault]) -> None:
    ...
    finish(
        ...
        output,
        "bug-order",
        "Rates describe the injected faults found in this synthetic population.",
    )
...
def run(argv: list[str] | None = None, *, workspace: FixtureWorkspace | None = None) -> int:
    ...
    with BenchmarkProgress("Discovery: interaction strengths") as display:
        for strength in display.track(range(1, args.max_strength + 1)):
