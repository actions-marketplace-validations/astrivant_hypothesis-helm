...
from hypothesis_helm_benchmarking.charts.topology import expected_topology
from hypothesis_helm_benchmarking.charts.workload import expected_output, source_digest
...
def run(argv: list[str] | None = None, *, workspace: FixtureWorkspace | None = None) -> int:
    ...
    with BenchmarkProgress("Sparsity: sample sizes") as display:
        for stage, indices in enumerate(display.track(stages), 1):
