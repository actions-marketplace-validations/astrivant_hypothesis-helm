...
`pipeline` schedules cooperative `pipeline.Workload` implementations. It ships alongside `pipeline` in this project's
...
by `None`. Existing `pipeline.Operation` commands expose statistics too, but remain non-preemptible in `OperationQueue`.
The existing refresh therefore retains its process ownership and measurement isolation; it is not silently switched to this scheduler.
...
```python
from pipeline import Control, Estimate, Outcome, Statistics, Work
...
Completed work removes its checkpoint. Restore loads paused units; callers reconstruct
the dynamic graph and account for already completed external work. Each scheduler requires its own directory; `scheduler.lock` prevents
...
a stale lock. Each scheduler coordinates work on a single host.
...
```python
from pathlib import Path
from pipeline import Graph, Scheduler
...
```python
from pipeline import FeedbackGraph
from pipeline import Estimate, Statistics, Work
...
rounds across activations. The round limit survives pause/resume. The application must change the workload
...
Callbacks must be idempotent, quick, nonblocking and independent of invocation order. Events record pending,
failed and completed finalizers. A grace deadline does not bypass them. If finalization is interrupted, the scheduler lock
remains until the operator completes recovery.
...
feedback rounds with stop conditions. Prerequisite graphs remain acyclic.
...
```bash
poetry install -E benchmarking
poetry run pipeline-example
```
...
The command prints its output directory under `.cache/pipeline/`. Override it with `--output <fresh-directory>`.
...
Each snapshot describes its local scheduling boundary; nested graphs have their own snapshots.
...
```python
from pipeline.gates import AND, OR, NOT, XOR, NXOR, Signal
