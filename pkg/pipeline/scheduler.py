...
class Scheduler:
    ...
    def _emit(self, kind: str, **detail: object) -> None:
        ...
        if kind != "statistics":
            self.notify(f"[pipeline] {kind}: {json.dumps(detail)}")
        records = {
        ...
        if self.plots and kind in {"graph_created", "graph_changed"}:
            from pipeline.plotting import plot_graph
    ...
    def run(self, control: Control | None = None) -> dict[str, str]:
        ...
        try:
            ...
            pool = ThreadPoolExecutor(max_workers=self.slots, thread_name_prefix="pipeline")
