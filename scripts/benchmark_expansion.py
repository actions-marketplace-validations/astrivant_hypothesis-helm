...
from scripts.benchmarking.structures import STRUCTURES
...
def compare(
    ...
    for strategy, raw_indices in mapping(reference["selected_indices"]).items():
        ...
        for enabled in (False, True):
            scheduler = FailureExpansion.build(chart.path, chart.defaults, values, values, initial)
            queue = list(initial)
            checked: list[int] = []
            found: set[int] = set()
            extra = 0
            ...
            rows.append(
                {
                    ...
                    "additional_indices": scheduler.added,
                    "unclassified_inputs": len(values) - len(scheduler.membership),
                    "inferred_failures": 0,
                }
            )
    return rows
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("docs/benchmarks/expansion"))
    ...
    from scripts.benchmarking.expansion_plots import plot
